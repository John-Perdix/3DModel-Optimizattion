...
remesher = 'DECIMATE'
remesh_percent = 0.5  # For DECIMATE, percentage of reduction (e.g., 1.0 means 1%)

RECALCULATE_LOW_POLY_NORMALS = True

LOW_POLY_UV_ISLAND_MARGIN = 0.001 # island_margin=0.05
...
def uv_unwrap_process():
    ...
    if method == "SMART":
        bpy.ops.object.editmode_toggle()
        bpy.ops.mesh.select_all(action="SELECT")
        bpy.ops.uv.smart_project(angle_limit=1, area_weight=0, island_margin=LOW_POLY_UV_ISLAND_MARGIN, correct_aspect=True, scale_to_bounds=True)
        bpy.ops.object.editmode_toggle()
